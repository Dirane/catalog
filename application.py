...
# Login Routing
# Login - Create anti-forgery state token oauth course 12/10/2017
@app.route('/login')
def displayLogin():
    state = ''.join(
...
def editCategory(category_name):
    ...
    if created_by.id != login_session['user_id']:
        flash ("You cannot edit this Category. This Category belongs to %s" % created_by.name)
        return redirect(url_for('displayCatalog'))
...
def deleteCategory(category_name):
    ...
    # If logged in user is not authorized
    if created_by.id != login_session['user_id']:
        flash ("You cannot delete this Category. This Category belongs to %s" % created_by.name)
        return redirect(url_for('displayCatalog'))
    if request.method =='POST':
...
def editItem(category_name, item_name):
    ...
    if created_by.id != login_session['user_id']:
        flash ("You are not %s. This item doesn't belong to you, editing forbiden" % created_by.name)
        return redirect(url_for('displayCatalog'))
...
# url_for static path processor credits
...
# Ato see if name is provided then run else name is main as defualt python
